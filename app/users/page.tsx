...
function Page() {
  ...
  const columns: TableProps<UserWithRole>["columns"] = [
    ...
    },
    {
      ...
      render: (_, record) =>
        hasPermission(user, "users", "edit-user", record) ? (
          ...
          <Space size="middle">
            <Tag style={{ marginLeft: 12 }} color={getTableLevelColor(record.level)}>
              {record.level}
            </Tag>
          </Space>
        ),
    },
    {
      ...
      minWidth: 150,
      render: (val, record) => {
        return hasPermission(user, "users", "edit-user", record) ? <EditCcaRolesSelect userRow={record} userList={userList} setEdit={setEdit} refetch={refetch} /> : <span style={{ marginLeft: 12 }}>{record.role.name}</span>
      },
    },
    {
      title: "Access Role",
      ...
      render: (_, record) =>
        hasPermission(user, "users", "edit-user-access-role", record) ? (
          ...
          <Space size="middle">
            <Tag style={{ marginLeft: 12 }} color={getTableAccessRoleColor(record.accessRole)}>
              {record.accessRole}
            </Tag>
          </Space>
        ),
  ...
  return (
    <div className="px-8 py-8 flex flex-col gap-4 max-w-screen-xl ml-0">
      ...
      <div className="p-4 md:px-8 md:py-6 border-2 border-grey-100 border-opacity-50 rounded-lg flex flex-col gap-2 md:gap-0">
        <Table<UserWithRole> columns={columns} dataSource={userList} rowKey="id" pagination={false} className="cursor-pointer" />
