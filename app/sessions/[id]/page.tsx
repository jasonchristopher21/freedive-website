        ...
        <div className="flex flex-col justify-center">
          <div className="flex gap-1.5">
            <span className={`${styles.heading3} font-bold text-black`}>
              {user.preferredName ? user.preferredName.toUpperCase() : user.name.split(" ")[0].toUpperCase()}
            </span>
            {isIc && (
              <div className="flex flex-col justify-center">
                <span className={`${styles.paragraph} text-white font-bold text-[11px] my-auto bg-blue-500 px-1 rounded-sm`}>IC</span>
              </div>
            )}
          </div>
          <span className="text-[11px] text-grey-500 -mt-0.5">{user.Role.name}</span>
        </div>
      </div>
    </div>
  )
...
export default function Page() {
  ...
  useEffect(() => {
    if (data && data.SessionIC && data.Signup) {
      ...
      const memberList = data.Signup
        .filter((signup: SignupObject) => !icUserIds.includes(signup.User.id))
        .map((signup: SignupObject) => ({
          ...signup.User, isIc: false
        }));
      const userList = data.SessionIC
        .map((ic: SignupObject) => ({
          ...ic.User, isIc: true
        })).concat(memberList);
      setUsers(userList);
        ...
        <div>
          <div className="px-4 py-6 flex flex-col gap-4 max-w-screen-xl mx-auto">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-2 p-4 border rounded-lg border-grey-300 max-w-screen-lg">
